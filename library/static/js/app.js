...
function googleBookSearch() {
    ...
    $.ajax({
        ...
        .done(function (data){
            ...
            <td><input name="pub_date" type="hidden" form="import-form${i}" readonly class="form-control-plaintext" value="${data.items[i].volumeInfo.publishedDate.split('-')[0]}">${data.items[i].volumeInfo.publishedDate.split('-')[0]}</td>
            <td><input name="isbn" type="hidden" form="import-form${i}" readonly class="form-control-plaintext" value="${data.items[i].volumeInfo.industryIdentifiers[0].identifier.replace(/\D/g, '')}">${data.items[i].volumeInfo.industryIdentifiers[0].identifier.replace(/\D/g, '')}</td>
